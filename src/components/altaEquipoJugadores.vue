...
<v-ons-page :infinite-scroll="loadMore">
  ...
  <v-ons-list>
    <v-ons-list-item v-for="(item, $index) in list" :key="item._id" tappable>
      <label class="left">
        <v-ons-checkbox :input-id="'checkbox-' + $index" :value="item._id" v-model="equipo.jugadores">
        </v-ons-checkbox>
      </label>
  ...
  <v-ons-speed-dial position="bottom right" direction="up" :visible="list.length > 0 || equipo.jugadores.length > 0" v-show="list.length > 0 || equipo.jugadores.length > 0" :open.sync="spdOpen">
    <v-ons-fab>{{equipo.jugadores.length}}</v-ons-fab>
